...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {   
        ...
        //using information from login form to prepare variables
        $email = htmlspecialchars(stripslashes(trim($_POST["email"])));         
        $password = htmlspecialchars(stripslashes(trim($_POST["password"])));   
        
        //validating email and get the users login credentials from the database
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {        
            $statement = $pdo->prepare("SELECT * FROM users WHERE email = :email");     
            $result = $statement->execute(array('email' => $email));
            $user = $statement->fetch();

            //setting users credentials as session variables if they match the credentials from the database
