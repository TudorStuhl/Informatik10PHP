...
if user is logged in: displaying user information and form to change credentials*/
...
if ($_SESSION["user_id"] == -1 && isset($_SESSION["user_id"])) {        
    ...
    if (array_key_exists("change_email", $_POST)) {    //form to set new email 
        echo
        '<form method="post" action="' .  htmlspecialchars($_SERVER["PHP_SELF"]) . '">
    ...
    else if (array_key_exists("change_username", $_POST)) {     //form to set new username
        echo
        '<form method="post" action="' .  htmlspecialchars($_SERVER["PHP_SELF"]) . '">
    ...
    if (array_key_exists("new_email", $_POST)) {    //validating email and call function "update_userdata"
    ...
    if (array_key_exists("new_username", $_POST)) {   //call function "update_userdata"
        update_userdata("username", $user_id, $_POST["new_username"]);     
    }
    echo '<a href="index.php">Zurück zur Startseite</a>';       
} 
    
//function to update userdata in database and refresh the session variables 
